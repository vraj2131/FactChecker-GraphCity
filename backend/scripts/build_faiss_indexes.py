...
# )
# from backend.app.utils.file_io import load_pickle, load_numpy
...
#     parser = argparse.ArgumentParser(description="Build FEVER claim embeddings + FAISS index (debug).")
...
#         "--use-sample",
#         action="store_true",
#         help="Use sample FEVER claims parquet instead of full claims parquet.",
#     )
#     parser.add_argument(
#         "--model-name",
#         type=str,
...
# def resolve_claims_path(args: argparse.Namespace) -> Path:
#     if args.claims_path:
...
#     _log("\n" + "=" * 70)
#     _log("PHASE 3 BUILD SUMMARY")
#     _log("=" * 70)
#     _log(f"Claims indexed:   {build_info['num_claims_indexed']:,}")
...
#     # Defensive: ensure 2D float32
#     if embeddings.ndim == 1:
#         embeddings = embeddings.reshape(1, -1)
#     embeddings = embeddings.astype(np.float32)

#     actual_queries = min(num_queries, len(metadata), embeddings.shape[0])
#     if actual_queries <= 0:
#         raise ValueError("No queries available for sanity check.")

#     query_embeddings = np.ascontiguousarray(embeddings[:actual_queries].astype(np.float32))

#     with _Timer(f"FAISS search (queries={actual_queries}, top_k={top_k})"):
...
#     # Print small output
#     for q_idx in range(actual_queries):
#         q = metadata[q_idx]
#         _log("-" * 70)
#         _log(f"QUERY {q_idx + 1} | claim_id={q.get('claim_id')}")
#         _log(f"text: {q.get('claim_text_normalized')}")
#         _log("Top neighbors:")
...
#                 n = metadata[n_idx]
#                 _log(f"  {rank+1}. idx={n_idx} score={score:.4f} claim_id={n.get('claim_id')} text={n.get('claim_text_normalized')}")
#             else:
...
#     claims_path = resolve_claims_path(args)
#     artifacts_dir = Path(args.artifacts_dir)
#     embeddings_dir = Path(args.embeddings_dir)

#     embeddings_output_path = embeddings_dir / FEVER_CLAIM_EMBEDDINGS_FILENAME
#     metadata_output_path = artifacts_dir / FEVER_CLAIM_METADATA_FILENAME
#     faiss_index_output_path = artifacts_dir / FEVER_CLAIMS_FAISS_INDEX_FILENAME

#     _log(f"[INFO] Claims parquet: {claims_path}")
...
#             raise FileNotFoundError(f"Claims parquet not found: {claims_path}")
...
#                 claims_parquet_path=claims_path,
...
#         # If no rebuild, infer embedding dim from the saved embeddings
...
#         build_info = {"embedding_dim": embedding_dim}

#     if args.skip_sanity_check:
#         _log("[INFO] Skipping sanity check as requested.")
#         _log("[DONE] Phase 3 completed (build only).")
#         return
...
#     _log("[DONE] Phase 3 claim index build + sanity check completed successfully.")
